...
function loadcustomer() {
    ...
    var ajaxConfig = {
        method: 'GET',
        url: 'http://localhost:8080/posweb/api/v1/customers' + '?size=' + 5 + '&page=' + pagenumber,
        //contentType:'application/json',
        async: true,
        //setRequestHeader:('X-Count','Content-Type','application/json'),

    };
    $.ajax(ajaxConfig).done(function (tbldata, status, jqXHR) {
       ...
       showOrHideFooter();
       var customerCount=jqXHR.getResponseHeader("X-Count");
...
$("#btnSubmit").click(function () {
    ...
    if ($("#btnSubmit").text() == "Save") {
        ...
        var ajaxConfig = {
            method:'POST',
            url: 'http://localhost:8080/posweb/api/v1/customers',
            async:true,
            contentType:'application/json',
            data:JSON.stringify(customer)
        };

        $.ajax(ajaxConfig).done(function (customer,status,jqSHR) {
            loadcustomer();
...
*/
        ...
        if (cusName.match("^[A-Za-z]+$") && cusAddress.match("^[A-Za-z]+$")) {
            ...
            $.ajax(ajaxConfig).done(function (customer,state,jqSHR) {
                loadcustomer();
            }).fail(function (jqSHR,state,error) {
                console.log(error);
            });
